...
new Vue({
  ...
  data: {
    ...
    u: {
      ...
      modalChangeGroupsVisible: false,
  ...
  created() {
    ...
    _this.$root.$on('u-change-groups', function () {
      _this.u.modalChangeGroupsVisible = true;
      _this.u.newGroups = _this.groups?.length ? _this.groups.split(', ') : [];
      var data = new URLSearchParams();
      data.append('username', _this.username);
      data.append('groups', _this.groups);
  ...
  computed: {
    ...
    groupChangeStatusCssClass: function () {
      return this.u.groupsChangeStatus === 200 ? "alert-success" : "alert-danger"
    },
    ...
    modalChangeGroupsDisplay: function () {
      return this.u.modalChangeGroupsVisible ? {display: 'flex'} : {}
    },
    ...
    groupOptions: function () {
      return this.groupsList.length ? this.groupsList : []
  ...
  methods: {
    ...
    rowActionFn: function (e) {
      ...
      this.groups = e.target.dataset.groups;
      this.$root.$emit(e.target.dataset.name);
      this.$root.$emit(e.target.dataset.groups);
    },
    getUserData: function () {
      var _this = this;
      axios.request(axios_cfg('api/users/list'))
        .then(function (response) {
          _this.rows = Array.isArray(response.data) ? response.data?.map(item => {


            return {...item, Groups: item.Groups?.join(', ')}
          }) : [];
        });
    },
    ...
    getGroupsData: function () {
      var _this = this;
      axios.request(axios_cfg('api/groups/list'))
        .then(function (response) {
          _this.groupsList = Array.isArray(response.data) ? response.data : [];
        });
    },
    ...
    changeUserGroups: function (user) {
      var _this = this;

      _this.u.groupsChangeMessage = "";

      axios.request(axios_cfg('api/user/groups/apply', JSON.stringify({
        'User': user,
        Groups: _this.u.newGroups
      }), 'json'))
        .then(function (response) {
          _this.u.groupsChangeStatus = 200;
          _this.u.newGroups = [];
          _this.getUserData();
          _this.u.modalChangeGroupsVisible = false;
          _this.$notify({title: 'Groups for user ' + _this.username + ' changed!', type: 'success'})
        })
        .catch(function (error) {
          _this.u.groupsChangeStatus = error.response.status;
          _this.u.groupsChangeMessage = error.response.data.message;
          _this.$notify({title: 'Changing groups for user ' + _this.username + ' failed!', type: 'error'})
        });
    },
